...
jdbc_driver = "/Users/rushipatel/Desktop/cs236_project/src/postgresql-42.7.3.jar"
...
def create_spark_session():
    ...
    spark = (
        SparkSession.builder.appName("Load Hotel Reservations Data to PostgreSQL")
...
def load_customer_reservations(spark):
    ...
    df = (
        ...
        .csv("data/customer-reservations.csv")
    )
...
def load_hotel_booking(spark):
    ...
    df.write.jdbc(
        url=jdbc_url, table="hotel_booking", mode="overwrite", properties=db_properties
    )
...
def load_merged_data(spark):
    ...
    df.write.jdbc(
        url=jdbc_url, table="merged_dataset", mode="overwrite", properties=db_properties
    )
...
def main():
    ...
    if not os.path.exists(jdbc_driver):
        ...
        print("Please make sure postgresql-42.7.3.jar exists in the src directory")
        return
